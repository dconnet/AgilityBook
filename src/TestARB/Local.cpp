...
bool CLocalization::Load()
{
	...
	m_InvalidRoot = fmt::sprintf(StringUtil::GetTranslation(arbT("IDS_INVALID_ROOT")), TREE_BOOK);
	m_MissingConfig = fmt::sprintf(StringUtil::GetTranslation(arbT("IDS_MISSING_CONFIG")), TREE_CONFIG);
	m_InvalidConfig = fmt::sprintf(StringUtil::GetTranslation(arbT("IDS_INVALID_CONFIG")), TREE_CONFIG);
	m_InvalidVenueConfig = fmt::sprintf(StringUtil::GetTranslation(arbT("IDS_INVALID_VENUE_CONFIG")), TREE_DIVISION, TREE_EVENT);
...
std::wstring CLocalization::UpdateCalSites(int nNew, int nUpdated, int nSkipped) const
{
	return fmt::format(L"{}: {}, {}, {}",
		...
		fmt::sprintf(StringUtil::GetTranslation(arbT("IDS_UPDATE_ADDED")), nNew),
		fmt::sprintf(StringUtil::GetTranslation(arbT("IDS_UPDATE_UPDATED")), nUpdated),
		fmt::sprintf(StringUtil::GetTranslation(arbT("IDS_UPDATE_IDENTICAL")), nSkipped));
}
...
std::wstring CLocalization::UpdateDivisions(int nAdded, int nUpdated, int nSkipped) const
{
	return fmt::format(L"{}: {}, {}, {}",
		...
		fmt::sprintf(StringUtil::GetTranslation(arbT("IDS_UPDATE_ADDED")), nAdded),
		fmt::sprintf(StringUtil::GetTranslation(arbT("IDS_UPDATE_UPDATED")), nUpdated),
		fmt::sprintf(StringUtil::GetTranslation(arbT("IDS_UPDATE_IDENTICAL")), nSkipped));
}
...
std::wstring CLocalization::UpdateMultiqs(int nAdded, int nDeleted, int nSkipped) const
{
	return fmt::format(L"{}: {}, {}, {}",
		...
		fmt::sprintf(StringUtil::GetTranslation(arbT("IDS_UPDATE_ADDED")), nAdded),
		fmt::sprintf(StringUtil::GetTranslation(arbT("IDS_UPDATE_DELETED")), nDeleted),
		fmt::sprintf(StringUtil::GetTranslation(arbT("IDS_UPDATE_IDENTICAL")), nSkipped));
}
...
{
	return fmt::sprintf(StringUtil::GetTranslation(arbT("IDS_ACTION_DELETE_CALPLUGIN")), name);
}
		...
		int nChanges) const
{
	...
	fmt::format_to(buffer, L"{}", fmt::sprintf(StringUtil::GetTranslation(arbT("IDS_ACTION_RENAME_OTHERPTS")), oldName, newName));
	if (0 < nChanges)
	{
		fmt::format_to(buffer, L"{}", fmt::sprintf(StringUtil::GetTranslation(arbT("IDS_ACTION_RENAME_OTHERPTS2")), nChanges));
	}
...
std::wstring CLocalization::ActionPreDeleteOtherPoints(
		...
		int nChanges) const
{
	return fmt::sprintf(StringUtil::GetTranslation(arbT("IDS_ACTION_PREDELETE_OTHERPTS")), name, nChanges);
}


std::wstring CLocalization::ActionDeleteOtherPoints(std::wstring const& name) const
{
	return fmt::sprintf(StringUtil::GetTranslation(arbT("IDS_ACTION_DELETE_OTHERPTS")), name);
}
...
{
	fmt::wmemory_buffer buffer;
	fmt::format_to(buffer, L"{}", fmt::sprintf(StringUtil::GetTranslation(arbT("IDS_ACTION_RENAME_VENUE")), oldName, newName));
	if (0 < nChanges)
	{
		fmt::format_to(buffer, L"{}", fmt::sprintf(StringUtil::GetTranslation(arbT("IDS_ACTION_RENAME_VENUE2")), nChanges));
	}
	return fmt::to_string(buffer);
}
...
std::wstring CLocalization::ActionPreDeleteVenue(
		...
		int nChanges) const
{
	return fmt::sprintf(StringUtil::GetTranslation(arbT("IDS_ACTION_PREDELETE_VENUE")), name, nChanges);
}


std::wstring CLocalization::ActionDeleteVenue(std::wstring const& name) const
{
	return fmt::sprintf(StringUtil::GetTranslation(arbT("IDS_ACTION_DELETE_VENUE")), name);
}
...
{
	fmt::wmemory_buffer buffer;
	fmt::format_to(buffer, L"{}", fmt::sprintf(StringUtil::GetTranslation(arbT("IDS_ACTION_RENAME_MULTIQ")), venue, oldName, newName));
	if (0 < nChanges)
	{
		fmt::format_to(buffer, L"{}", fmt::sprintf(StringUtil::GetTranslation(arbT("IDS_ACTION_RENAME_MULTIQ2")), nChanges));
	}
	return fmt::to_string(buffer);
}
...
std::wstring CLocalization::ActionPreDeleteMultiQ(
		...
		int nChanges) const
{
	return fmt::sprintf(StringUtil::GetTranslation(arbT("IDS_ACTION_PREDELETE_MULTIQ")), venue, name, nChanges);
}
...
{
	return fmt::sprintf(StringUtil::GetTranslation(arbT("IDS_ACTION_DELETE_MULTIQ")), venue, name);
}
...
{
	fmt::wmemory_buffer buffer;
	fmt::format_to(buffer, L"{}", fmt::sprintf(StringUtil::GetTranslation(arbT("IDS_ACTION_RENAME_DIV")), venue, oldName, newName));
	if (0 < nChanges)
	{
		fmt::format_to(buffer, L"{}", fmt::sprintf(StringUtil::GetTranslation(arbT("IDS_ACTION_RENAME_DIV2")), nChanges));
	}
	return fmt::to_string(buffer);
}
...
std::wstring CLocalization::ActionPreDeleteDivision(
		...
		int nChanges) const
{
	return fmt::sprintf(StringUtil::GetTranslation(arbT("IDS_ACTION_PREDELETE_DIV")), venue, name, nChanges);
}
...
std::wstring CLocalization::ActionDeleteDivision(
		...
		std::wstring const& name) const
{
	return fmt::sprintf(StringUtil::GetTranslation(arbT("IDS_ACTION_DELETE_DIV")), venue, name);
}
...
{
	fmt::wmemory_buffer buffer;
	fmt::format_to(buffer, L"{}", fmt::sprintf(StringUtil::GetTranslation(arbT("IDS_ACTION_RENAME_LEVEL")), venue, oldName, newName));
	if (0 < nChanges)
	{
		fmt::format_to(buffer, L"{}", fmt::sprintf(StringUtil::GetTranslation(arbT("IDS_ACTION_RENAME_LEVEL2")), nChanges));
	}
	return fmt::to_string(buffer);
}
...
std::wstring CLocalization::ActionPreDeleteLevel(
		...
		int nChanges) const
{
	return fmt::sprintf(StringUtil::GetTranslation(arbT("IDS_ACTION_PREDELETE_LEVEL")), venue, name, nChanges);
}
...
std::wstring CLocalization::ActionDeleteLevel(
		...
		std::wstring const& name) const
{
	return fmt::sprintf(StringUtil::GetTranslation(arbT("IDS_ACTION_DELETE_LEVEL")), venue, name);
}
...
{
	fmt::wmemory_buffer buffer;
	fmt::format_to(buffer, L"{}", fmt::sprintf(StringUtil::GetTranslation(arbT("IDS_ACTION_RENAME_TITLE")), venue, oldName, newName));
	if (0 < nChanges)
	{
		fmt::format_to(buffer, L"{}", fmt::sprintf(StringUtil::GetTranslation(arbT("IDS_ACTION_RENAME_TITLE2")), nChanges));
	}
	return fmt::to_string(buffer);
}
...
std::wstring CLocalization::ActionPreDeleteTitle(
		...
		int nChanges) const
{
	return fmt::sprintf(StringUtil::GetTranslation(arbT("IDS_ACTION_PREDELETE_TITLE")), venue, name, nChanges);
}
...
std::wstring CLocalization::ActionDeleteTitle(
		...
		std::wstring const& name) const
{
	return fmt::sprintf(StringUtil::GetTranslation(arbT("IDS_ACTION_DELETE_TITLE")), venue, name);
}
...
{
	fmt::wmemory_buffer buffer;
	fmt::format_to(buffer, L"{}", fmt::sprintf(StringUtil::GetTranslation(arbT("IDS_ACTION_RENAME_EVENT")), venue, oldName, newName));
	if (0 < nChanges)
	{
		fmt::format_to(buffer, L"{}", fmt::sprintf(StringUtil::GetTranslation(arbT("IDS_ACTION_RENAME_EVENT2")), nChanges));
	}
	return fmt::to_string(buffer);
}
...
std::wstring CLocalization::ActionPreDeleteEvent(
		...
		int nChanges) const
{
	return fmt::sprintf(StringUtil::GetTranslation(arbT("IDS_ACTION_PREDELETE_EVENT")), venue, name, nChanges);
}
...
std::wstring CLocalization::ActionDeleteEvent(
		...
		std::wstring const& name) const
{
	return fmt::sprintf(StringUtil::GetTranslation(arbT("IDS_ACTION_DELETE_EVENT")), venue, name);
}
...
{
	if (0 == nEventChanges)
		return fmt::sprintf(StringUtil::GetTranslation(arbT("IDS_ACTION_RENAME_LIFETIMENAME_0")), venue, oldName, newName);
	else
		return fmt::sprintf(StringUtil::GetTranslation(arbT("IDS_ACTION_RENAME_LIFETIMENAME_N")), venue, oldName, newName, nEventChanges);
}
...
{
	if (0 == nEventChanges)
		return fmt::sprintf(StringUtil::GetTranslation(arbT("IDS_ACTION_DELETE_LIFETIMENAME_0")), venue, name);
	else
		return fmt::sprintf(StringUtil::GetTranslation(arbT("IDS_ACTION_DELETE_LIFETIMENAME_N")), venue, name, nEventChanges);
}


std::wstring CLocalization::TitlePointsNameFormat(double points, double faults) const
{
	return fmt::sprintf(StringUtil::GetTranslation(arbT("IDS_TITLEPOINTSNAME")), points, faults);
}


std::wstring CLocalization::LifetimePointsNameWithSpeedPointsFormat(double faults) const
{
	return fmt::sprintf(StringUtil::GetTranslation(arbT("IDS_LIFETITLEPOINTSNAME")), StringUtil::GetTranslation(arbT("IDS_COL_SPEED")), faults);
}


std::wstring CLocalization::LifetimePointsNameFormat(double points, double faults) const
{
	std::wstring dbl1 = fmt::sprintf(L"%g", points);
	return fmt::sprintf(StringUtil::GetTranslation(arbT("IDS_LIFETITLEPOINTSNAME")), dbl1, faults);
}
...
{
	std::wstring dbl1 = fmt::sprintf(L"%g", points);
	return fmt::sprintf(StringUtil::GetTranslation(arbT("IDS_PLACEMENTPOINTSNAME")), dbl1, (int)place);
}


std::wstring CLocalization::WarnDeletedRuns(int nRuns, std::wstring const& inRunsMsg) const
{
	return fmt::format(L"{}\n{}", fmt::sprintf(StringUtil::GetTranslation(arbT("IDS_WARN_DELETED_RUNS")), nRuns), inRunsMsg);
}


std::wstring CLocalization::UpdateTeamRuns(int nRuns, std::wstring const& inRunsMsg) const
{
	return fmt::format(L"{}\n{}", fmt::sprintf(StringUtil::GetTranslation(arbT("IDS_UPDATE_TEAM_RUNS")), nRuns), inRunsMsg);
}


std::wstring CLocalization::UpdateTableRuns(int nRuns) const
{
	return fmt::sprintf(StringUtil::GetTranslation(arbT("IDS_UPDATE_TABLE_RUNS")), nRuns);
}
