...
LifeTimePointInfoPtr LifeTimePointInfo::New(
		...
		double inFiltered)
{
	return make_shared<LifeTimePointInfo_concrete>(inSort1, inSort2, inPoints, inFiltered);
}
...
void CPointsDataItems::InsertVenueHeader(
		...
		ARBConfigVenuePtr pVenue)
{
	m_Lines.push_back(make_shared<CPointsDataText>(pDoc, false));
	m_Lines.push_back(make_shared<CPointsDataVenue>(pDoc, inDog, pVenue));
}
		...
		ARBDogPtr inDog)
{
	...
	m_Lines.push_back(make_shared<CPointsDataDog>(pDoc, inDog));
		...
		++iterVenue)
	{
			...
			++iterTitle)
		{
			...
			&& !pTitle->IsFiltered())
			{
				if (!bHeaderInserted)
				{
					...
					data += L"</h3>";
					data += s_TableHeader;
					m_Lines.push_back(make_shared<CPointsDataSeparator>(pDoc, data));
				}
				m_Lines.push_back(make_shared<CPointsDataTitle>(pDoc, inDog, pTitle));
			}
		}
		if (bHeaderInserted)
			m_Lines.push_back(make_shared<CPointsDataSeparator>(pDoc, L"</table>"));
		...
		|| 0 < trialsInVenue.size())
		{
				...
				++idxDiv, ++iterDiv)
			{
				...
				if (bHasSpeedPts)
				{
					speedPtsData.push_back(make_shared<CPointsDataSpeedPts>(pDoc, pVenue, pDiv, speedPts));
				}
			} // division loop
			if (1 < items.size())
			...
			if (0 < pVenue->GetMultiQs().size())
			{
				...
				for (std::map<ARBConfigMultiQPtr, std::set<MultiQdata> >::iterator iter = MQs.begin();
					...
					++iter)
				{
					m_Lines.push_back(make_shared<CPointsDataMultiQs>(pDoc, inDog, pVenue, (*iter).first, (*iter).second));
				}
			}
		}
	...
	if (0 < other.size())
	{
		...
		m_Lines.push_back(make_shared<CPointsDataSeparator>(pDoc, table));
		m_Lines.push_back(make_shared<CPointsDataText>(pDoc, false));
		m_Lines.push_back(make_shared<CPointsDataText>(pDoc, false, str.c_str()));
			...
			++iterOther)
		{
			...
			case ARBConfigOtherPoints::eTallyAll:
				m_Lines.push_back(make_shared<CPointsDataOtherPointsTallyAll>(pDoc, pOther->GetName(), runs));
				break;

			case ARBConfigOtherPoints::eTallyAllByEvent:
				m_Lines.push_back(make_shared<CPointsDataText>(pDoc, true, L"", pOther->GetName().c_str()));
				{
						...
						++iterTally)
					{
						...
						m_Lines.push_back(make_shared<CPointsDataOtherPointsTallyAllByEvent>(pDoc, (*iterTally), validRuns));
					}
				}
				break;
			...
			case ARBConfigOtherPoints::eTallyLevel:
				m_Lines.push_back(make_shared<CPointsDataText>(pDoc, true, L"", pOther->GetName().c_str()));
				{
						...
						++iterTally)
					{
						...
						m_Lines.push_back(make_shared<CPointsDataOtherPointsTallyLevel>(pDoc, (*iterTally), validRuns));
					}
				}
				break;
			...
			case ARBConfigOtherPoints::eTallyLevelByEvent:
				m_Lines.push_back(make_shared<CPointsDataText>(pDoc, true, L"", pOther->GetName().c_str()));
				{
						...
						++iterTally)
					{
						...
						m_Lines.push_back(make_shared<CPointsDataOtherPointsTallyLevelByEvent>(pDoc, (*iterTally).first, (*iterTally).second, validRuns));
					}
				}
				break;
			}
		}
		m_Lines.push_back(make_shared<CPointsDataSeparator>(pDoc, L"</table></html>"));
	}
}
