		...
		int border)
{
	...
	m_Ctrl->Connect(wxEVT_COMMAND_LIST_KEY_DOWN, wxListEventHandler(CAgilityBookPointsView::OnCtrlKeyDown), NULL, this);
...
CAgilityBookPointsViewDataPtr CAgilityBookPointsView::GetItemPointsData(long index) const
{
	...
	return tr1::dynamic_pointer_cast<CAgilityBookPointsViewData, CListData>(m_Ctrl->GetData(index));
}
...
void CAgilityBookPointsView::LoadData()
{
	...
	for (size_t nItem = 0; nItem < nItems; ++nItem)
	{
		...
		if (data->IsVisible())
		{
			long item = m_Ctrl->InsertItem(CAgilityBookPointsViewDataPtr(new CAgilityBookPointsViewData(data, m_Ctrl->ImageEmpty())));
...
void CAgilityBookPointsView::OnCtrlKeyDown(wxListEvent& evt)
{
