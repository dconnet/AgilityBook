 ...
 *
 * The MD5 code has combined and simplified several headers and source into
 * this one file.  This includes modifying the existing routines to take
 * const pointer (to avoid reinterpret_cast) and reordered functions to avoid
 * prototypes. (Also, using C++ notation instead of C) [all comments in the
 * RFC code are original, I only tweaked comment blocks, except where I note
...
// These are the original external interfaces into the MD5 code.
