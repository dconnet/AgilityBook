...
int _tmain(int argc, TCHAR* argv[])
{
	...
	{
		CStringA pgm(argv[0]);
		cerr << "Usage: " << (LPCSTR)pgm << " resource.h resource.hm Help/AgilityBook.txt output/AgilityBook.h output/contextid.h" << endl;
		return 1;
	}
	CStringA resourceH(argv[argc-5]);
	CStringA resourceHM(argv[argc-4]);
	CStringA agilitybookTXT(argv[argc-3]);
	CStringA agilitybookH(argv[argc-2]);
	CStringA contextidH(argv[argc-1]);
	...
	ifstream resH((LPCSTR)resourceH);
	if (!resH.is_open())
	{
		cerr << "Can't read " << (LPCSTR)resourceH << endl;
		return 1;
	}
	ofstream contextH((LPCSTR)contextidH);
	if (!contextH.is_open())
	{
		cerr << "Can't write " << (LPCSTR)contextidH << endl;
		return 1;
	}

	contextH << "// Autogenerated from " << (LPCSTR)resourceH << endl;
	while (!resH.eof())
	{
	...
	ifstream resHM((LPCSTR)resourceHM);
	if (!resHM.is_open())
	{
		cerr << "Can't read " << (LPCSTR)resourceHM << endl;
		return 1;
	}
	ofstream arbH((LPCSTR)agilitybookH);
	if (!arbH.is_open())
	{
		cerr << "Can't write " << (LPCSTR)agilitybookH << endl;
		return 1;
	}

	set<string> existingHelpIds;
	arbH << "// Autogenerated from " << (LPCSTR)resourceHM  << endl;
	while (!resHM.eof())
	{
	...
	ifstream arbTXT((LPCSTR)agilitybookTXT);
	if (!arbTXT.is_open())
	{
		cerr << "Can't read " << (LPCSTR)agilitybookTXT << endl;
		return 1;
	}
	while (!arbTXT.eof())
	{
		...
		if (0 == pos)
		{
			...
			if (!str.IsEmpty())
			{
				...
				if (existingHelpIds.end() == i)
					cerr << "Warning: " << s << " is defined in " << (LPCSTR)agilitybookTXT << " but has no definition in " << (LPCSTR)resourceHM << endl;
				else
	...
	{
		cerr << "Warning: " << *i << " is defined in " << (LPCSTR)resourceHM << " but has no help topic in " << (LPCSTR)agilitybookTXT << endl;
	}
