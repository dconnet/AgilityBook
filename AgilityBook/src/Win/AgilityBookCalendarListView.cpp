...
void CAgilityBookCalendarListView::LoadData()
{
	...
	++iter)
	{
		...
		long index = m_Ctrl->InsertItem(CAgilityBookCalendarListViewDataPtr(new CAgilityBookCalendarListViewData(this, pCal)));
	...
	case wxID_COPY:
		{
			...
			if (0 < m_Ctrl->GetSelection(indices))
			{
				...
				clpData.AddData(eFormatiCalendar, wxString(tstringUtil::Convert(iCal.str()).c_str()));
				clpData.CommitData();
			}
		}
		break;
